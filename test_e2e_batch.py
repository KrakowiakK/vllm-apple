#!/usr/bin/env python3
"""End-to-end test for vLLM Apple with real model inference.
...
def test_batch_inference(batch_sizes=[1, 2, 4, 8, 16]):
    ...
    print("=" * 70)
    print("vLLM Apple Silicon - End-to-End Batch Test")
    print("=" * 70)
    ...
    for batch_size in batch_sizes:
        ...
        for i, output in enumerate(outputs):
            prompt = output.prompt[:50] + "..." if len(output.prompt) > 50 else output.prompt
            response = output.outputs[0].text.strip()[:80]
